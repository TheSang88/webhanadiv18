    ...
    class MY_Controller extends CI_Controller
    {
        function __construct()
        {
           ...
           // $tuantruoc = strtotime( date('Y-m-d',strtotime('-7 day')));
            ...
            $this->optionCart = $this->system_model->getFirstRowWhere('configDatHang',array('lang' => $this->language));
            $url = base_url();
            $this->domain = parse_url($url);
            // echo "<pre>";
            // print_r($this->optionCart);
            // echo "<pre>";
            // $http = 'http' . ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') ? 's' : '') . '://';
            // $newurl = str_replace("index.php","", $_SERVER['SCRIPT_NAME']);
            // $BaseURL = "$http" . $_SERVER['SERVER_NAME'] . "" . $newurl;
            // if($_SERVER['SCRIPT_NAME']!=='localhost/demo_mypham_17_8_183/'){
        ...
        public function LoadHeader($view,$seo=array(),$show_slider=false)
        {
            ...
            $tong_daily=$this->system_model->count_daily();
           
            $data['tong_daily'] =$tong_daily;
            
            //$data['count'] = $this->count_cart;
            $items = @$_SESSION['cartPro'];
          $data['count'] = 0;
        if(isset($items) && !empty($items)){
            foreach ($items as $key => $value) {
               $data['count'] += $value['qty'];
            }
        }
            $data['menu_main_right'] = $this->system_model->get_data('menu',array('position'=>'top','parent_id'=>0,'lang' => $this->language),
                   array('sort'=>'')
               );
            foreach ($data['menu_main_right'] as $key => $cat) {
                $data['menu_main_right'][$key]->menu_sub =  $this->system_model->get_data('menu',array( 'position'=>'main',
                'parent_id ='=>$cat->id,
                'lang' => $this->language),
                    array('sort'=>''));
            }
            ...
            foreach ($data['menu_main_root'] as $key => $cat) {
                $data['menu_main_root'][$key]->menu_sub  = $menu_sub = $this->system_model->get_data('menu',array( 'position'=>'main',
                'parent_id ='=>$cat->id,
                'lang' => $this->language),
                    array('sort'=>''));
                foreach ($data['menu_main_root'][$key]->menu_sub as $key2 => $menu_sub) {
                    $menu_sub->menu_sub_2 =  $this->system_model->get_data('menu',array(
                    'parent_id ='=>$menu_sub->id,
                    'lang' => $this->language),
                        array('sort'=>''));
                }
        ...
        public function LoadFooter($view = null)
        {
            ...
            foreach ($data['menu_bottom'] as $key => $menu_bottom) {
             ...
             $data['top'] = $this->system_model->get_data('images',array(
            ...
            //news mới nhất
            $data['news_new_footer'] = $this->system_model->get_data('news',array(
