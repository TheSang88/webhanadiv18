<div class="modal fade bs-example-modal-lg popup1" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	...
	<div class="modal-dialog modal-lg">
		<?php if(isset($key)){ ?>
		<form class="validate form-horizontal" role="form" id="form-bk" method="POST" action="<?= base_url('wpadmin/admin/edit_thuoctinh/' . $key . '') ?>" enctype="multipart/form-data">
			<input type="hidden" name="edit" id="id_edit" value="<?= @$key; ?>">
		<?php }else{ ?>
		<form class="validate form-horizontal" role="form" id="form-bk" method="POST" action="<?= base_url('wpadmin/admin/add_attpro/') ?>" enctype="multipart/form-data">
		 <?php } ?>
