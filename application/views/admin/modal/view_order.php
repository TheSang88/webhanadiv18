				...
				<table class="table table-bordered">
					<tbody>
						...
						foreach($detail_order as $d){
							$tootle+=$d->price*$d->qty;
							$nameSize = !empty($d->nameSize)?'<br>'.$d->nameSize:'';
							?>
						<tr>
							...
							<td><a target="_blank" href="<?=base_url('san-pham/'.$d->alias.'.html')?>"><?=$d->name;?><?=$nameSize;?></a></td>
							<td class="text-right"><?=$d->qty;?></td>
							<td class="text-right"><?=number_format($d->price);?></td>
							<td colspan="2" class="text-right"><?=number_format($d->price*$d->qty);?></td>
						</tr>
						<?php } ?>
						<tr>
