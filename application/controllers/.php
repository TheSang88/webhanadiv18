                ...
                if (isset($id)) {
                    $config = Array(
                        ...
                        if ($this->email->send()) {
                            ...
                            $this->session->set_flashdata("mess_err", "Đăng ký thành công! Chúng tôi sẽ duyệt tài khoản thời gian sớm nhất");
...
// thông tin tai khoan
    public function acount()
    {
        ...
        $this->LoadHeader('common/header',$seo);
   ...
   // thay doi mat khau
     public function change_pass(){
       ...
       $this->LoadHeader('common/header',$seo);
        $this->load->view('users/doimatkhau',$data);
        $this->LoadFooter();
    ...
    // thay doi mat khau
     public function danhsachtin(){
        ...
        $where = array(
                    ...
                    'aid' => $data['user_item']->email

                );

                $config['base_url'] = base_url('wpadmin/news/newslist');
                ...
                $data['list'] = $this->news_model->getListNews_rao($data['user_item']->email, $config['per_page'],
       ...
       $this->LoadHeader('common/header',$seo);
        $this->load->view('users/danhsachtin',$data);
        $this->LoadFooter();
           ...
           // $check = false;
            ...
            //if($user->password == $current_pass){
               // $check = true;
                ...
                for($i=0;$i<5;$i++){
                    $pass=md5(md5($change_pass));
                }
              
                $bien = $this->f_usersmodel->Update_where('users',array('id'=>$user->id),array(
                    'password' => $pass,
                ...
                $this->session->set_flashdata("mess", "Bạn đã đổi mật khẩu thành công!!!");
                redirect($_SERVER['HTTP_REFERER']);
