...
//                    $total_sale= $subtotal*$code_sale/100;
                ...
                $img ='<p><img style="width:140px" src="'.base_url(@$this->option->site_logo).'" alt=""/></p>';
                ...
                //$this->email->send();
                   ...
                   $title = $subject;
                    ...
                    $nTo = $subject;
                    $mTo = "info@Hanadi.vn";
                    $diachicc = $this->option->site_email.','.$_POST['email'];
                    $diachiBcc = '';
                    $Reply = @$this->option->site_email;
                    $mFrom = "feconfms2022@gmail.com";
                    $mPass = "edvelgvwhjydcvif";
                    if (!empty($mFrom) && !empty($mPass)) {
                        $mail = sendMail2($title, $content, $nTo, $mTo, $diachicc, $nFrom = $this->option->site_name, $Reply, $mFrom, $mPass);
                    }
...
// danh hang thanh cong
 public function order_success(){
    ...
    public function sendOnpage2()
    {
       
        if($this->input->post('form_tocken') !='' && $this->input->post('form_tocken') == $this->session->userdata('form_key'))
        {
            ...
            $data['cart'] = $v = $this->cartmodel->getFirstRowWhere('product',array(
                'id' => $this->input->post('id')));
            ...
            $arr = array(
                ...
                'code_sale' => $this->input->post('coupon'),
                'price_ship' => $this->input->post('price_shipping'),
                'total_price' => $this->input->post('qty')*$v->price_sale,
                'mobile' => $this->input->post('phone_other'),
                'startplaces' => $payment,
                'time' => time(),
                'user_id' => @$user_info->id,
            ...
            $id=$this->cartmodel->Add('order',$arr);
                if($id)
                {
                    ...
                    $detai_order=array(
                        ...
                        'name' => $v->name,
                        'subtotal' => $this->input->post('qty')*$v->price_sale,
                        'pro_dir' => $v->pro_dir,
                        'alias' => $v->alias,
                        'image' => $v->image,
                        //'size' => $v['size'],
                ...
                $htm = '<table width="100%" border="1" cellpadding="5" cellspacing="0" bordercolor="#caf6ea">
                    ...
                    $tongtien = $this->input->post('qty')*$v->price_sale;
                    $htm .='<tr>';
                    $htm .='<td> 1 </td>';
                    $htm .='<td>'.$v->name.'<br>';
                    $htm .='</td>';
                    $htm .='<td>'.$this->input->post('qty').'</td>';
                    $htm .='<td>'.number_format($v->price_sale).'</td>';
                    $htm .='<td>'.number_format($this->input->post('qty')*$v->price_sale).'</td>';
                    $htm .='</tr>';
                

                $htm .='<tr><td colspan="5" align="right"><span style="color:red">Tổng tiền đơn hàng:'.number_format($tongtien).'&nbsp;vnđ</span></td></tr>';
                $htm .='<tr><td colspan="5" align="right"><span style="color:red">Tổng tiền thanh toán là:'.number_format($tongtien).'&nbsp;vnđ</span></td></tr>';
                $htm .='</tbody>';
                $htm .='</table>';
