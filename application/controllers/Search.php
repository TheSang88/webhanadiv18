...
class Search extends MY_Controller
{
    ...
    public function searchNews(){
       ...
       $data = array();
        ...
        $config['enable_query_strings'] = TRUE;
         ...
         $config=array_merge($config,$this->pagination_config);
         $this->pagination->initialize($config);
         $data['lists'] = $this->m_search->getNews_search($where,$config['per_page'],
             $this->input->get('per_page'));
         foreach ($data['lists'] as $k => $v){
             $data['lists'][$k]->comment = $this->m_search->Count('comments_binhluan',array('id_sanpham'=>$v->id,'review'=>1));
         }
        ...
        if($xuatxu !='')
        {
            if($cat != ''){
                ...
                foreach($arr_xuatxu as $lc){
                    $where_xuatxu .="product_category.id = ". $cat ." and product_locale.id = '".$lc."' or ";
                }
                ...
                $arr_xuatxu = explode(",",$xuatxu);
                foreach($arr_xuatxu as $lc){
                    $where_xuatxu .=" product_locale.id = '".$lc."' or ";
                }
                $where_xuatxu = rtrim($where_xuatxu,'or ');
                $whereAll .= ' and ' .$where_xuatxu;
            }
    ...
    public function filter(){
        ...
        $offset = ($page - 1) * $number_per_page;
         $data['lists'] = $lists = $this->m_search->getFillProduct($where,$number_per_page,$offset,1);
        $data['total_rows'] = $this->m_search->getFillProduct($where,$number_per_page,$offset,2);
        $data['phantrang'] = createPaginationFontend($data['total_rows'], $number_per_page, $page);
        $this->load->view('searchs/filter',$data);
    }
    public function filter_daily(){
        $data = array();
        $catid = $this->input->post('id');
         $huyen = $this->input->post('thuong_hieu'); // huyen
        $tinhthanh = $this->input->post('khoang_gia'); //tỉnh thành
        

         if($tinhthanh !='')
        {

             $where_dk .="media.tinhthanh = ".$tinhthanh. "";
         ...
         if($huyen !='')
        {
            ...
            // $where_dk .="and media.huyen = ".$huyen. "";

        }

        $data['lists'] = $this->m_search->ghetmedia($where_dk);

        //var_dump($data['lists']);

        $this->load->view('searchs/view_filter_daily',$data);
    }
