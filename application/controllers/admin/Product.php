...
class Product extends MY_Controller
{
    function __construct()
    {
        ...
        {
            redirect(base_url().'wpadmin', 'location');
            die();
        }
...
//    ======================================================================================================================
    public function products()
    {
        ...
        if($this->input->get()){
            ...
            $config['base_url']             = base_url('wpadmin/product/products?code='
                . $this->input->get('code')
                . '&name=' . $this->input->get('name')
                . '&cate=' . $this->input->get('cate')
                . '&view=' . $this->input->get('view')
            ...
            $config['base_url'] = base_url('wpadmin/product/products');
    ...
    public function add($id_edit=null)
    {
        ...
        if($id_edit!=null){
            ...
            if(!empty($data['array_json']) && isset( $arr1)){
            ...
            // $data['season_selected'] = $this->productmodel->getField_array('product_to_season','id_season',
                // array('id_product'=>$id_edit));
            $data['sizeCheckeds'] = $sizeCheckeds = $this->productmodel->getField_array('product_to_size','*',
                 array('id_product'=>$id_edit));
            if(isset($sizeCheckeds) && !empty($sizeCheckeds)){
                foreach ($sizeCheckeds as $key => $value) {
                    $data['sizeCheckeds'][$value['id_size']] = $data['sizeCheckeds'][$key];
                    unset($data['sizeCheckeds'][$key]);
                }
            }
            // echo "<pre>";
            // print_r($data['sizecheckeds']);
            // echo "</pre>";
        ...
        if (isset($_POST['addnews'])) {
            ...
            $code_check = $this->productmodel->getFirstRowWhere('product',array(
          ...
          if (isset($_POST['content']) && sizeof($_POST['content']) > 0) {
            ...
            $arr = array(
                ...
                'contents'     => count($this->input->post('contente'))?$this->input->post('contente'):'',
                'code'            => $this->input->post('code'),
            ...
            if (!empty($_POST['edit'])){
                ...
                $id = $this->productmodel->Update_where('product', array('id'=>$id_edit), $arr);
                 $this->productmodel->Update_where('product',array('id' => $id_edit),array(
                    'time_update' =>time()
                ));
            ...
            // }
            //========================Cập nhập giá theo size
             $this->productmodel->Update_where('product_to_size',array('id_product'=>$id), array('active'=>2));
            if (isset($_POST['active']) && sizeof($_POST['active']) > 0) {
                for ($i = 0; $i < sizeof($_POST['active']); $i++) {
                    $idSize = $_POST['active'][$i];
                    $arrSizes = array('id_product'      =>$id,
                                        'id_size'  =>str_replace(",", "", $_POST['id_size_'.$idSize]),
                                        'priceNiemYet'  =>str_replace(",", "", $_POST['priceNiemYet_'.$idSize]),
                                        'priceSale'     =>str_replace(",", "", $_POST['priceSale_'.$idSize]),
                                        'statusSize'  =>str_replace(",", "", $_POST['statusSize_'.$idSize]),
                                        'active'=>1
                                        );
                    
                    if($_POST['id_'.$idSize]){
                        $this->productmodel->Update('product_to_size',$_POST['id_'.$idSize], $arrSizes);
                    }else{
                        $this->productmodel->Add('product_to_size', $arrSizes);
                    }
                }
            }
            ...
            $config['allowed_types'] = 'gif|jpg|png|jpeg|JPEG|JPG|PNG|GIF|webp|';
            $config['max_size'] = '*';
            ...
            $config2['allowed_types'] = 'gif|jpg|png|jpeg|JPEG|JPG|PNG|GIF|webp';
            $config2['max_size'] = '*';
            ...
            redirect(base_url('wpadmin/product/products'));


            // }else{ 

                
             //    echo "<script>alert('Không thêm được dữ liệu, do mã sản phẩm trùng');</script>";
        ...
        foreach ($this->session->userdata('phanquyen') as $key => $cat) {
         ...
         $data['sizes'] = $this->productmodel->get_data('product_size',array('lang' => $this->language ),null);
        // hien thi danh sach nut
        $data['show_home'] = $this->productmodel->getFirstRowWhere('config_checkpro',array('type' => 'product','field' => 'home',));
        $data['show_hot'] = $this->productmodel->getFirstRowWhere('config_checkpro',array('type' => 'product','field' => 'hot',));
        $data['show_focus'] = $this->productmodel->getFirstRowWhere('config_checkpro',array('type' => 'product','field' => 'focus',));
      ...
      public function images_add()
    {
       //$this->check_acl();
        ...
        if (isset($_POST['Upload']))
        {
          ...
          redirect(base_url('wpadmin/product/images/'.$id));
        }
...
//******* danh muc ********************************************************
    ...
    public function cat_add($id_edit=null)
    {
        ...
        $config['allowed_types'] = 'gif|jpg|jpeg|png|GIF|JPG|JPEG|PNG|webp';
        ...
        if (isset($_POST['addnews'])) {
            ...
            if(!empty($_FILES['download'])){
                    for($i=0;$i<count($_FILES['download']["name"]);$i++)
                        {
                            if(!empty($_FILES['download']["tmp_name"][$i])){
                               $uploadfile=$_FILES['download']["tmp_name"][$i];
                             $folder="upload/img/category/";
                             $type_image = explode(".", $_FILES['download']['name'][$i]);
                            $name_file = ($type_image[0]).'.'.$type_image[1];
                             move_uploaded_file($_FILES['download']['tmp_name'][$i], $folder.$name_file);
                             
                              $this->productmodel->Update_where('product_category', array('id'=>$id), array('banner'=>$folder.$name_file));
                              
                            }
                           
                        }
                }
            redirect(base_url('wpadmin/product/categories'));
        }
        $where = array('lang' => $this->language);
        if($id_edit){
            $where['id !='] = $id_edit;
        }
        $data['cate'] = $this->productmodel->get_data('product_category',$where,array('sort'=>'asc'));
         $data['show_home'] = $this->productmodel->getFirstRowWhere('config_checkpro',array('type' => 'product_category','field' => 'home',));
        $data['show_hot'] = $this->productmodel->getFirstRowWhere('config_checkpro',array('type' => 'product_category','field' => 'hot',));
        $data['show_focus'] = $this->productmodel->getFirstRowWhere('config_checkpro',array('type' => 'product_category','field' => 'focus',));
        $data['show_coupon'] = $this->productmodel->getFirstRowWhere('config_checkpro',array('type' => 'product_category','field' => 'coupon',));
