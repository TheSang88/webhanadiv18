...

    {
        ...
        if (isset($_POST['addnews'])) {
           ...
           if($id){
            ...
            redirect(base_url('wpadmin/attribute/listBrand'));
...
//=========== locale ==================================================================================================
...

    {
        ...
        if (isset($_POST['addnews'])) {
            ...
            redirect(base_url('wpadmin/attribute/listLocale'));
...
public function listColor()
...

    {
        ...
        if (isset($_POST['addnews'])) {
            ...
            redirect(base_url('wpadmin/attribute/listColor'));
 ...
 public function listprice()
...

    {
        ...
        if (isset($_POST['addnews'])) {
            ...
            redirect(base_url('wpadmin/attribute/listprice'));
...
//            'product_type_id' => 1
        ...
        $data['product_type']=$this->m_attribute->get_data('product_type',array(),array());


        $data['headerTitle'] = 'Quản lý dung tích';
...

    {
        ...
        if (isset($_POST['addnews'])) {
            ...
            redirect(base_url('wpadmin/attribute/listOption'));
    ...
    public function listOptionType()

    {
        ...
        $data['headerTitle'] = 'Loại sản phẩm 1 giá';

        $this->LoadHeaderAdmin($data);

        $this->load->view('admin/product_size/list_type', $data);

        $this->load->view('admin/footer');

    }

    public function addoptionType($id_edit=null)

    {
        ...
        if (isset($_POST['addnews'])) {
            ...
            redirect(base_url('wpadmin/attribute/listOptionType'));
        ...
        $this->load->view('admin/product_size/add_type', $data);

        $this->load->view('admin/footer');

    }

    public function editoptionType($id=null)
    ...
    public function product_dvt() {
            $data = array();
            $where = array();
            $where['name'] = $this->input->get('name');
            $where['table'] = 'product_dvt';
            $config['page_query_string']    = TRUE;
            $config['enable_query_strings'] = TRUE;
            $config['total_rows']           =  count($this->productmodel->getTableByName($where));
            $config['per_page']             = 20;
            $config['uri_segment'] = 4;
            if($this->input->get()){
                $config['base_url'] = base_url('wpadmin/attribute/product_dvt?'
            ...
            $data['list'] = $this->productmodel->getTableByName($where,$config['per_page'], $this->input->get('per_page'));
            $data['name'] = 'Cách đóng gói';



            $this->LoadHeaderAdmin($data);
            $this->load->view('admin/product_size/product_dvt', $data);
            $this->load->view('admin/footer');
    ...
    public function product_type() {
        ...
        $config['uri_segment'] = 4;
        if($this->input->get()){
            $config['base_url'] = base_url('wpadmin/attribute/product_type?'
                . '&name=' . $this->input->get('name')
            );
        } else {
            $config['base_url'] = base_url('wpadmin/attribute/product_type');
        ...
        $this->LoadHeaderAdmin($data);
        $this->load->view('admin/product_size/product_type', $data);
        $this->load->view('admin/footer');
