...
class Users extends MY_Controller
{
    ...
    public function listusers(){
		...
		if($this->input->get()){
            ...
            $config['base_url']             = base_url('wpadmin/users/listusers?phone='
            ...
            $config['base_url'] = base_url('wpadmin/users/listusers');
    ...
    public function smslist(){
        ...
        $config['base_url'] = base_url('wpadmin/users/smslist');
...
public function listuser_admin(){
		if($this->input->get()){
            ...
            $config['page_query_string']    = TRUE;
            $config['enable_query_strings'] = TRUE;
            $config['base_url']             = base_url('wpadmin/users/listuser_admin?phone='
                . $this->input->get('phone')
                . '&name=' . $this->input->get('fullname')
                . '&email=' . $this->input->get('email')
				. '&view=' . $this->input->get('view')
            ...
            $config['base_url'] = base_url('wpadmin/users/listuser_admin');
            $config['total_rows'] = $this->usersmodel->count_All('users'); // xác định tổng số record
            $config['per_page'] =20; // xác định số record ở mỗi trang
            $config['uri_segment'] = 4; // xác định segment chứa page number
            $config=array_merge($config,$this->pagination_config);
...
//            $data['max_sort']=$data['edit']->sort;
        ...
        if(isset($_POST['adduser']))
        {
            ...
            redirect(base_url('wpadmin/users/listuser_admin'));
        }
