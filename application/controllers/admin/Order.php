...
public function addSale($id_edit=null)
    {
        ...
        if (isset($_POST['addnews'])) {
            ...
            redirect(base_url('wpadmin/product_sale/listSale'));
        }
...
// quan ly phi van chuyen ship
    public function listProvince()
    {
     ...
     public function configCart()
    {
        //$this->check_acl();
       if (isset($_POST['add_news'])) {
            $arr = array(
                'emailGui' => $this->input->post('emailGui'),
                'maBaomat' => $this->input->post('maBaomat'),
                'emailTraLoi' => $this->input->post('emailTraLoi'),
                'emailNhanDatHang' => $this->input->post('emailNhanDatHang'),
                'emailDatHang' => $this->input->post('emailDatHang'),
                'lang'=>$this->language
            );
            // echo "<pre>";
            // print_r($arr);
            // echo "<pre>";
            // die();
            if (isset($this->optionCart) && !empty($this->optionCart)){
               
                $id = $this->ordermodel->Update_where('configDatHang', array('lang'=>$this->language), $arr);
                $this->session->set_flashdata("mess", "Cập nhật thành công!");
            } else {
               
                $id = $this->ordermodel->Add('configDatHang', $arr);
                $this->session->set_flashdata("mess", "Thêm mới thành công!");
            }
            redirect(base_url('wpadmin/order/configCart'));
        }
        $data['headerTitle'] = "Cấu hình thông tin đặt hàng ngoài website";
        $this->LoadHeaderAdmin($data);
        $this->load->view('admin/order/configCart', $data);
        $this->load->view('admin/footer');
    }
     public function configSuccess()
    {
        //$this->check_acl();
       if (isset($_POST['add_news'])) {
            $arr = array(
                'thongBaoDatHangThanhCong' => $this->input->post('thongBaoDatHangThanhCong'),                
                'lang'=>$this->language
            );
            // echo "<pre>";
            // print_r($arr);
            ...
            redirect(base_url('wpadmin/order/configSuccess'));
        }
        $data['headerTitle'] = "Cấu hình thông tin đặt hàng ngoài website";
        $this->LoadHeaderAdmin($data);
        $this->load->view('admin/order/configSuccess', $data);
        $this->load->view('admin/footer');
    }
}