...
class Ajax extends MY_Controller
{
    ...
    public function changeSize(){
        ...
        $html = 'Liên hệ';
        $arr['check'] = 0;
        $arr['class'] = 'outOfStock';
        $arr['text'] = 'Hết hàng';
        $item = $this->system_model->getField('product_to_size','priceNiemYet,priceSale,statusSize,active',array('id'=>$id));
        if(isset($item) && !empty($item)){
            $priceNiemYet = $item->priceNiemYet;
            $priceSale = $item->priceSale;
            $statusSize = $item->statusSize;
            $active =$item->active;
            if($priceSale >0 && $statusSize == 1 && $active ==1){
                if($priceNiemYet > 0 && $priceNiemYet > $priceSale){
                    $html = '<span class="priceSale">'.number_format($priceSale,0,",",".").' đ </span>
                            <span class="price">'.number_format($priceNiemYet,0,",",".").' đ</span>';
                }else{
                    $html = number_format($priceSale,0,",",".").' đ';
                }
                $arr['check'] = 1;
                $arr['class'] = '';
                $arr['text'] = 'Còn hàng';
    ...
    public function delete_fillter(){
        ...
        $check = false;
        if(count($item)){
            $id = $this->adminmodel->Update_where($table,array('id'=>$id),array($fill=>''));
           // xoa anh san pham
            if(file_exists($link)){
                @unlink($link);
            }
