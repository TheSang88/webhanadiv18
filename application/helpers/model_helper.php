...
function sendMail2($title, $content, $nTo, $mTo,$diachicc='',$nFrom='',$Reply,$mFrom,$mPass){
    include('phpmail/class.smtp.php');
    include "phpmail/class.phpmailer.php"; 
    $nFrom = $nFrom;
    $mFrom ="feconfms2022@gmail.com";
    $mPass = "edvelgvwhjydcvif";
    $mail             = new PHPMailer();
    $body             = $content;
    $mail->IsSMTP(); 
    $mail->CharSet   = "utf-8";
        ...
        if ($v->parent_id == $parent) {
            ...
            echo "<tr>
					...
					<td class='text-center'>
						...
						<a href='".base_url('wpadmin/group/deletecategory/' . $v->id)."'
    ...
    foreach ($data as $k=>$v) {
        if ($v->parent_id == $parent) {
            ...
            echo "<tr>
                ...
                echo"<td class='text-center'>
					<a href='".base_url('wpadmin/news/cat_edit/' . $v->id)."'
                        class=\"btn btn-xs btn-default\" title=\"Sửa\"><i class=\"fa fa-pencil\"></i></a>
					<a href='".base_url('wpadmin/news/deletecategory/' . $v->id)."'
                       onclick=\"return confirm('Bạn có chắc chắn muốn xóa?')\"
                       class=\"btn btn-xs btn-danger\"title=\"Xóa\" style=\"color: #fff\"><i class=\"fa fa-times\"></i> </a>
                    </td>
                </tr>";
    ...
    foreach ($data as $k=>$v) {
        if ($v->parent_id == $parent) {
            ...
            echo "<tr>
                    ...
                    <td><a href='".base_url('wpadmin/product/cat_edit/' . $v->id)."'
            ...
            echo   "<td class='text-center'>
					<a href='".base_url('wpadmin/product/cat_edit/' . $v->id)."'
                        class=\"btn btn-xs btn-default\" title=\"Sửa\"><i class=\"fa fa-pencil\"></i></a>
					<a href='".base_url('wpadmin/product/deletecategory/' . $v->id)."'
                       onclick=\"return confirm('Bạn có chắc chắn muốn xóa?')\"
                       class=\"btn btn-xs btn-danger\"title=\"Xóa\" style=\"color: #fff\"><i class=\"fa fa-times\"></i> </a>
                    </td>
                </tr>";
    ...
    foreach ($data as $k=>$v) {
        if ($v->parent_id == $parent) {
            ...
            echo "<tr>
                ...
                <td class='text-center'>
					<a href='".base_url('wpadmin/raovat/cat_edit/' . $v->id)."'
                        class=\"btn btn-xs btn-default\" title=\"Sửa\"><i class=\"fa fa-pencil\"></i></a>
					<a href='".base_url('wpadmin/raovat/deletecategory/' . $v->id)."'
                       onclick=\"return confirm('Bạn có chắc chắn muốn xóa?')\"
                       class=\"btn btn-xs btn-danger\"title=\"Xóa\" style=\"color: #fff\"><i class=\"fa fa-times\"></i> </a>
                    </td>
                </tr>";
    ...
    foreach ($data as $k=>$v) {
        if ($v->parent_id == $parent) {
            ...
            echo "<tr>
                ...
                echo"<td class='text-center'>
					<a href='".base_url('wpadmin/media/cat_edit/' . $v->id)."'
                        class=\"btn btn-xs btn-default\" title=\"Sửa\"><i class=\"fa fa-pencil\"></i></a>
					<a href='".base_url('wpadmin/media/deletecategory/' . $v->id)."'
                       onclick=\"return confirm('Bạn có chắc chắn muốn xóa?')\"
                       class=\"btn btn-xs btn-danger\"title=\"Xóa\" style=\"color: #fff\"><i class=\"fa fa-times\"></i> </a>
                    </td>
                </tr>";
        ...
        if ($v->parent_id == $parent) {
            ...
            echo "<tr>
                ...
                if($show_home->active!=0 || $show_hot->active!=0 || $show_focus->active!=0){    
					...
					echo"<td class='text-center'>
						<a href='".base_url('wpadmin/video/cat_edit/' . $v->id)."'
							class=\"btn btn-xs btn-default\" title=\"Sửa\"><i class=\"fa fa-pencil\"></i></a>
						<a href='".base_url('wpadmin/video/deletecategory/' . $v->id)."'
						   onclick=\"return confirm('Bạn có chắc chắn muốn xóa?')\"
						   class=\"btn btn-xs btn-danger\"title=\"Xóa\" style=\"color: #fff\"><i class=\"fa fa-times\"></i> </a>
					</td>
				</tr>";
    ...
    foreach ($data as $k=>$v) {
        if ($v->parent_id == $parent) {
            ...
            echo "<tr>
                    ...
                    <td>".$text.$v->name."</td>
                    <td class='text-center'>
                        <a href='".base_url('wpadmin/admin/document_edit/' . $v->id)."'
                            class=\"btn btn-xs btn-default\" title=\"Sửa\"><i class=\"fa fa-pencil\"></i></a>
                        <a href='".base_url('wpadmin/admin/deletedocument/' . $v->id)."'
                           onclick=\"return confirm('Bạn có chắc chắn muốn xóa?')\"
                           class=\"btn btn-xs btn-danger\"title=\"Xóa\" style=\"color: #fff\"><i class=\"fa fa-times\"></i> </a>
                    </td>
                </tr>";
								...
								<div class="dd-handle" data-items="id_'.$t.'">' . $v->name . '</div>
								 ...
								 <div class="btn-group link_v">
									...
									<ul class="dropdown-menu" style="min-width: 40px !important; padding: 5px">
									  <li><a href="'.base_url('wpadmin/menu/edit/' . $v->id.'?p='.$v->position.'').'" style="color: #0011ca">Sửa</a></li>
									  <li><a href="'.base_url('wpadmin/menu/delete/' . $v->id).'"style="color: red"onclick="return confirm(\'Bạn có chắc chắn muốn xóa?\')" >Xóa</a></li>
									</ul>
								  </div>
								 </div>
								<ol class="dd-list">';
...
function sendMail($title, $content, $nTo, $mTo,$diachicc='',$mFrom='vvtuyen12@gmail.com',$mPass='ahjgnpmvqfdlkxty',$nFrom='chichi.vn',$ReplyTo='',$addressBcc=''){
    include('phpmail/class.smtp.php');
    include "phpmail/class.phpmailer.php"; 
   // $nFrom = 'chichi.vn';
    //$mFrom = 'hethong.noreply888@gmail.com';  //dia chi email cua ban 
    //$mPass = 'mapqwcomdmctxday';       //mat khau email cua ban
    $mail             = new PHPMailer();
    $body             = $content;
    $mail->IsSMTP(); 
    $mail->CharSet   = "utf-8";
    $mail->SMTPDebug  = 0;                     // enables SMTP debug information (for testing)
    $mail->SMTPAuth   = true;                    // enable SMTP authentication
    $mail->SMTPSecure = "tls";  //tls | ssl              // sets the prefix to the servier
    $mail->Host       = "smtp.gmail.com";        
    ...
    //$mail->AddAddress('vvtuyen12@gmail.com', $nTo);
    //echo $diachicc;
...
function createPaginationFontend($total, $number_per_page, $current_page = 1, $class='paginate_button', $classCurrent = 'paginate_active'){
            $pagination_system = '';
            $pagination_stages = 2;
            //echo $current_page;
            ...
            // Start
            if($last_page > 1){
                $pagination_system .='<div class="dataTable_length hidden">';
                ...
                if ($last_page < 7 + ($pagination_stages * 2)){
                    ...
                    for ($page_counter = 1; $page_counter <= $last_page; $page_counter++){
                        if ($page_counter == $current_page) {
                            $pagination_system.= "<li class='paginate_active'><a class='paginate_active'>$page_counter</a></li>";
                        }else {
                    ...
                    if($current_page < 1 + ($pagination_stages * 2)){
                        ...
                        for($page_counter = $last_page - (2+($pagination_stages*2)); $page_counter <= $last_page; $page_counter++){
                            if ($page_counter == $current_page) {
                                ...
                                $pagination_system.= "<li class='paginate_active'><a class=\"paginate_button\"  page='".$page_counter."'>$page_counter</a></li>";
                            }
                        }
                        $pagination_system .='</span>';
                    }
